...
import { saveLink, deleteLink, updateLink, addTagToLink, removeTagFromLink } from '../api/links';
...
function LinkList() {
  const { links, loading, error, refetch, silentRefetch } = useLinks();
  ...
  const [skipDeleteConfirm, setSkipDeleteConfirm] = useState(
    () => localStorage.getItem('folio_skip_delete_confirm') === 'true'
  );
  ...
  const executeDelete = async (linkId) => {
    try {
      await deleteLink(linkId);
      if (selectedLink && selectedLink.id === linkId) setSelectedLink(null);
      await refetch();
    } catch (err) {
  ...
  const handleConfirmDelete = (dontAskAgain) => {
    if (dontAskAgain) {
      localStorage.setItem('folio_skip_delete_confirm', 'true');
  ...
  // Fetch all tags for autocomplete whenever the panel opens
  useEffect(() => {
    if (selectedLink) {
  ...
  const handleAddTag = async (name, type) => {
    // Optimistic update
    const tempTag = { id: `temp-${Date.now()}`, name: name.toLowerCase(), tag_type: type };
    setSelectedLink(prev => ({ ...prev, tags: [...(prev.tags || []), tempTag] }));
    try {
      const { data: newTag } = await addTagToLink(selectedLink.id, { name, tag_type: type });
      // Replace temp tag with real one and refresh allTags pool
      setSelectedLink(prev => ({
        ...prev,
        tags: prev.tags.map(t => t.id === tempTag.id ? newTag : t),
      }));
      setAllTags(prev => prev.some(t => t.id === newTag.id) ? prev : [...prev, newTag]);
      silentRefetch();
    } catch {
      setSelectedLink(prev => ({ ...prev, tags: (prev.tags || []).filter(t => t.id !== tempTag.id) }));
    }
  };

  const handleRemoveTag = async (tag) => {
    // Optimistic update
    setSelectedLink(prev => ({ ...prev, tags: (prev.tags || []).filter(t => t.id !== tag.id) }));
    try {
      await removeTagFromLink(selectedLink.id, tag.id);
      // Tag may have been deleted server-side if orphaned — remove from pool too
      getTags().then(r => setAllTags(r.data)).catch(() => {});
      silentRefetch();
    } catch {
      setSelectedLink(prev => ({ ...prev, tags: [...(prev.tags || []), tag] }));
    }
  };
    ...
    <div className="min-h-screen bg-[#f7f8f5] text-[#26312d]">
      ...
      {confirmDeleteId !== null && (
        <ConfirmModal
          ...
          message="This will permanently remove the link and all its tags from your library. This can't be undone."
          confirmLabel="Delete"
          icon="trash"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {getBroadTags(selectedLink.tags).map((tag) => {
                    const tagName = getTagName(tag);
                    return (
                      <span key={tag.id || tagName} className="group/tag inline-flex items-center gap-1 font-mono px-2.5 py-0.5 rounded-full text-[10px] bg-[#edf4ef] text-[#315f56] border border-[#c8d8cf]">
                        <button onClick={(e) => handleTagClick(tagName, e)} className="hover:underline underline-offset-2">
                          {tagName}
                        </button>
                        <button
                          onClick={() => handleRemoveTag(tag)}
                          className="opacity-0 group-hover/tag:opacity-100 -mr-0.5 transition-opacity hover:text-red-400"
                          title="Remove tag"
                        >
                  ...
                  <TagInput
                    type="broad"
                    allTags={allTags}
                    existingIds={new Set(selectedLink.tags?.map(t => t.id) || [])}
                    onAdd={handleAddTag}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {getSpecificTags(selectedLink.tags).map((tag) => {
                    const tagName = getTagName(tag);
                    return (
                      <span key={tag.id || tagName} className="group/tag inline-flex items-center gap-1 font-mono px-2.5 py-0.5 rounded-full text-[10px] bg-white text-[#68746f] border border-[#d8ded8]">
                        <span>{tagName}</span>
                        <button
                          onClick={() => handleRemoveTag(tag)}
                          className="opacity-0 group-hover/tag:opacity-100 -mr-0.5 transition-opacity hover:text-red-400"
